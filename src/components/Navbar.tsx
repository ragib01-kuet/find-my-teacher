        ...
        <div className="hidden items-center gap-3 md:flex">
          {user ? (
            <>
              {role === "tutor" && (
                <Link to="/my-profile">
                  <Button variant="ghost" size="sm" className="gap-2"><User className="h-4 w-4" /></Button>
                </Link>
              )}
              {role === "student" && (
                <Link to="/dashboard">
                  <Button variant="ghost" size="sm" className="gap-2"><LayoutDashboard className="h-4 w-4" /></Button>
                </Link>
              )}
              <Link to="/messages" className="relative">
                <Button variant="ghost" size="sm" className="gap-2"><MessageCircle className="h-4 w-4" /></Button>
                {unreadCount > 0 && (
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="container flex flex-col gap-2 py-4">
              ...
              {user ? (
                <Button variant="outline" onClick={() => { signOut(); setMobileOpen(false); }} className="mt-2">Sign Out</Button>
              ) : (
